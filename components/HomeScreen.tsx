...
const PackageCard: FC<{ 
    ...
    showBorder?: boolean;
}> = ({ name, price, texts, onBuy, icon: Icon, description, diamonds, isSpecial, isPremium, size = 'normal', showBorder = true }) => {
    ...
    // Use softer gradient border by lowering opacity of the colors
    ...
    return (
        <div 
            className={`group relative rounded-2xl shadow-md hover:shadow-xl hover:shadow-primary/10 hover:-translate-y-1 transition-all duration-300 h-full overflow-hidden
                ${showBorder ? `p-[1px] ${borderGradient}` : 'p-0 bg-light-card dark:bg-dark-card'}
            `}
        >
            <div className={`rounded-[calc(1rem-1px)] ${s.padding} flex flex-col items-center justify-between h-full text-center relative overflow-hidden
                ${showBorder ? 'bg-light-card dark:bg-dark-card' : ''}
            `}>
                ...
                <div className="flex flex-col items-center justify-center flex-grow py-1 w-full">
                    {/* ICON AREA: Background and padding removed to float directly on card */}
                    ...
                    <div className="w-full">
                        {/* Title: Ensured not-italic (Normal) */}
                        <h3 className={`${s.titleSize} font-black text-light-text dark:text-dark-text tracking-tight line-clamp-2 ${s.minHeight} flex items-center justify-center not-italic`}>
                            {name}
                        </h3>
                    ...
                    {/* Description: Italic removed (fixing Diamonds text and Premium desc) */}
                    {description && <p className={`${s.descSize} text-gray-500 dark:text-gray-400 font-bold line-clamp-1 mt-0.5 not-italic`}>{description}</p>}
                </div>
                ...
                <div className="w-full mt-1 flex flex-col items-center">
                    ...
                    <button 
                        onClick={onBuy} 
                        className={`w-full text-white font-black ${s.btnSize} rounded-lg transition-all active:scale-95 shadow-lg 
                        ${isSpecial 
        ...
        case 'diamonds':
            return diamondOffers.map((offer, index) => (
              <div key={offer.id} className="opacity-0 animate-smart-slide-down" style={{ animationDelay: `${index * 80}ms` }}>
                  <PackageCard name={`${offer.diamonds}`} description={texts.diamondsLabel} price={offer.price} texts={texts} icon={DiamondIcon} size={cardSize} showBorder={showCardBorder} onBuy={() => handleBuyClick({id: offer.id, name: `${offer.diamonds} ${texts.diamondsLabel}`, price: offer.price, icon: DiamondIcon, diamonds: offer.diamonds, inputType: 'uid'})} />
              </div>
            ));
        case 'level-up':
            return levelUpPackages.map((pkg, index) => (
                 <div key={pkg.id} className="opacity-0 animate-smart-slide-down" style={{ animationDelay: `${index * 80}ms` }}>
                    <PackageCard name={texts[pkg.name] || pkg.name} price={pkg.price} texts={texts} icon={StarIcon} size={cardSize} showBorder={showCardBorder} onBuy={() => handleBuyClick({id: pkg.id, name: texts[pkg.name] || pkg.name, price: pkg.price, icon: StarIcon, inputType: 'uid'})} />
                </div>
            ));
        case 'memberships':
            return memberships.map((mem, index) => (
                <div key={mem.id} className="opacity-0 animate-smart-slide-down" style={{ animationDelay: `${index * 80}ms` }}>
                    <PackageCard name={texts[mem.name] || mem.name} price={mem.price} texts={texts} icon={IdCardIcon} size={cardSize} showBorder={showCardBorder} onBuy={() => handleBuyClick({id: mem.id, name: texts[mem.name] || mem.name, price: mem.price, icon: IdCardIcon, inputType: 'uid'})} />
                </div>
            ));
        case 'special':
        ...
        case 'premium-apps':
            return premiumApps.map((app, index) => (
                <div key={app.id} className="opacity-0 animate-smart-slide-down" style={{ animationDelay: `${index * 80}ms` }}>
                    <PackageCard name={app.name} description={app.description} price={app.price} texts={texts} icon={CrownIcon} size={cardSize} isPremium={true} showBorder={showCardBorder} onBuy={() => handleBuyClick({id: app.id, name: app.name, price: app.price, icon: CrownIcon, inputType: 'email'})} />
                </div>
            ));
        default:
            return null;
